...
class TaskContextTests(PySparkTestCase):
    ...
    def test_stage_id(self):
        """tests the stage ids are available and incrementing as expected."""
        rdd = self.sc.parallelize(range(10))
        stage1 = rdd.map(lambda x: TaskContext.get().stageId()).take(1)[0]
        stage2 = rdd.map(lambda x: TaskContext.get().stageId()).take(1)[0]
        # tests using the constructor directly rather than the get()
...
class RDDTests(ReusedPySparkTestCase):
    ...
    def test_sampling_default_seed(self):
        # tests for SPARK-3995 (default seed setting)
        data = self.sc.parallelize(xrange(1000), 1)
        subset = data.takeSample(False, 10)
        self.assertEqual(len(subset), 10)

    def test_aggregate_mutable_zero_value(self):
        # tests for SPARK-9021; uses aggregate and treeAggregate to build dict
    ...
    def test_aggregate_by_key_mutable_zero_value(self):
        # tests for SPARK-9021; uses aggregateByKey to make a pair RDD that
    ...
    def test_fold_mutable_zero_value(self):
        # tests for SPARK-9021; uses fold to merge an RDD of dict counters into
        # a single dict
        # NOTE: dict is used instead of collections.Counter for Python 2.6
        # compatibility
        from collections import defaultdict
    ...
    def test_fold_by_key_mutable_zero_value(self):
        # tests for SPARK-9021; uses foldByKey to make a pair RDD that contains
