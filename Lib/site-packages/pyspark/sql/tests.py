...
class SQLTests(ReusedSQLTestCase):
    ...
    def test_nondeterministic_udf(self):
        # tests that nondeterministic UDFs are evaluated only once in chained UDF evaluations
    ...
    def test_array_types(self):
        ...
        #
        # tests max, min, and precision for float and double, assuming IEEE 754
    ...
    def test_bucketed_write(self):
        ...
        # tests write with one bucketing column
        df.write.bucketBy(3, "x").mode("overwrite").saveAsTable("pyspark_bucket")
        self.assertEqual(count_bucketed_cols(["x"]), 1)
        self.assertSetEqual(set(data), set(self.spark.table("pyspark_bucket").collect()))

        # tests write two bucketing columns
        df.write.bucketBy(3, "x", "y").mode("overwrite").saveAsTable("pyspark_bucket")
        self.assertEqual(count_bucketed_cols(["x", "y"]), 2)
        self.assertSetEqual(set(data), set(self.spark.table("pyspark_bucket").collect()))

        # tests write with bucket and sort
        df.write.bucketBy(2, "x").sortBy("z").mode("overwrite").saveAsTable("pyspark_bucket")
        self.assertEqual(count_bucketed_cols(["x"]), 1)
        self.assertSetEqual(set(data), set(self.spark.table("pyspark_bucket").collect()))

        # tests write with a list of columns
        df.write.bucketBy(3, ["x", "y"]).mode("overwrite").saveAsTable("pyspark_bucket")
        self.assertEqual(count_bucketed_cols(["x", "y"]), 2)
        self.assertSetEqual(set(data), set(self.spark.table("pyspark_bucket").collect()))

        # tests write with bucket and sort with a list of columns
        (df.write.bucketBy(2, "x")
            .sortBy(["y", "z"])
            .mode("overwrite").saveAsTable("pyspark_bucket"))
        self.assertSetEqual(set(data), set(self.spark.table("pyspark_bucket").collect()))

        # tests write with bucket and sort with multiple columns
        (df.write.bucketBy(2, "x")
            .sortBy("y", "z")
            .mode("overwrite").saveAsTable("pyspark_bucket"))
        self.assertSetEqual(set(data), set(self.spark.table("pyspark_bucket").collect()))
...
class HiveContextSQLTests(ReusedPySparkTestCase):
    ...
    def test_window_functions_cumulative_sum(self):
        ...
        # tests boundary values less than JVM's Long.MinValue and make sure we don't overflow
        sel = df.select(
            df.key,
        ...
        # tests boundary values greater than JVM's Long.MaxValue and make sure we don't overflow
...
class ArrowTests(ReusedSQLTestCase):
    ...
    def test_createDataFrame_with_names(self):
        pdf = self.create_pandas_data_frame()
        # tests that schema as a list of column names gets applied
        df = self.spark.createDataFrame(pdf, schema=list('abcdefgh'))
        self.assertEquals(df.schema.fieldNames(), list('abcdefgh'))
        # tests that schema as tuple of column names gets applied
