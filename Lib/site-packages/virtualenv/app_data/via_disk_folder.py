...
└─── unzip <in zip modules we cannot refer to some internal files, so first extract them>
     └── <virtualenv version>
         ├── py_info.py
         ├── debug.py
         └── _virtualenv.py
...
class AppDataDiskFolder(AppData):
    ...
    def reset(self):
        logging.debug("reset modules data folder %s", self.lock.path)
