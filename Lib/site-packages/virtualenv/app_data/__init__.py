...
def make_app_data(folder, **kwargs):
    ...
    if not os.path.isdir(folder):
        try:
            os.makedirs(folder)
            logging.debug("created modules data folder %s", folder)
        except OSError as exception:
            logging.info("could not create modules data folder %s due to %r", folder, exception)

    if os.access(folder, os.W_OK):
        return AppDataDiskFolder(folder)
    else:
        logging.debug("modules data folder %s has no write access", folder)
        return TempAppData()
