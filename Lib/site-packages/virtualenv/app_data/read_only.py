...
class ReadOnlyAppData(AppDataDiskFolder):
    ...
    def __init__(self, folder):  # type: (str) -> None
        if not os.path.isdir(folder):
            raise RuntimeError("read-only modules data directory {} does not exist".format(folder))
        self.lock = NoOpFileLock(folder)

    def reset(self):  # type: () -> None
        raise RuntimeError("read-only modules data does not support reset")
...
class _PyInfoStoreDiskReadOnly(PyInfoStoreDisk):
    def write(self, content):
        raise RuntimeError("read-only modules data python info cannot be updated")
